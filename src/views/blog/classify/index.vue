  ...
  <div class="app-container">
    <el-row :gutter="20">
      <el-col>
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              ...
              size="mini"
              @click="addContent"
              v-if="checkPermission(['sys:blogClassify:add'])"
            >新增
            </el-button>
          </el-col>
        ...
        <el-table ref="tables" v-loading="loading" :data="list" :default-sort="defaultSort" @sort-change="handleSortChange">
          <el-table-column label="ID" align="center" width="70" prop="classifyId" />
          ...
          <el-table-column label="状态" align="center">
            <template slot-scope="scope">
              <el-switch
                ...
                :disabled="!checkPermission(['sys:blogClassify:enable', 'sys:blogClassify:disable'])"
          ...
          <el-table-column label="操作" align="center" width="180" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-button
                v-if="checkPermission(['sys:blogClassify:update'])"
                size="mini"
                type="text"
                icon="el-icon-edit"
              ...
              <el-button
                v-if="checkPermission(['sys:blogClassify:delete'])"
                size="mini"
                type="text"
                icon="el-icon-delete"
    ...
    <el-dialog :title="title" :visible.sync="open" :center="true" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules">
        <el-row>
          <el-col :span="24">
            <el-form-item label="分类内容" prop="title">
              <el-input v-model="form.title" placeholder="请输入分类内容" @keyup.enter.native="submitForm"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-col :span="24 ">
...
<script>
import { list, save, update, deleteById, getById, updateStatus } from '@/api/blog/classify/index'
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      const id = row.classifyId
      this.$confirm('确认要删除"' + row.title + '"分类吗？', '系统提示', {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.classifyId != undefined) {
            update(this.form.classifyId, this.form).then(response => {
    ...
    handleStatusChange(row) {
      ...
      this.$confirm('确认要' + text + '"' + row.title + '"分类吗？', '系统提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(function() {
